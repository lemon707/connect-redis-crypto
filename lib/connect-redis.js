/*
 * CUSTOMIZE: comments refer to changes made on top of connect-redis-crypto module
 */
/*!
 * Connect - Redis
...
module.exports = function (session) {
  ...
  function encrypt(key, pt, algo, encodeas) {
    //CUSTOMIZE: STRINGIFY PLAIN TEXT
  ...
  RedisStore.prototype.get = function (sid, fn) {
    ...
    store.client.get(psid, function (er, data) {
      ...
      //CUSTOMIZE: PARSE RESULT
      return fn(null, JSON.parse(result));
    });
  };
  ...
  RedisStore.prototype.set = function (sid, sess, fn) {
    ...
    try {
      var jsess = store.serializer.stringify((store.secret) ?
        encryptData.call(store, sess) : sess);
    }
    catch (er) {
      return fn(er);
    }
    ...
    if (this.secret)
      //CUSTOMIZE: COMMENT OUT SECND ENCRYPTDATA
      // jsess = encryptData.call(this, jsess, this.secret, this.algorithm);

    //CUSTOMIZE: STRINGIFY THE JSESS OBJECT
    args.push(JSON.stringify(jsess));
