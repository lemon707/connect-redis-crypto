...
module.exports = function (session) {
  ...
  function encryptData(plaintext){
    var pt = encrypt(this.secret, plaintext, this.algo, this.encodeas)
  ...
  function decryptData(ciphertext){
    ...
    return decrypt(this.secret, ciphertext.ct, this.algo, this.encodeas);
  }
  ...
  RedisStore.prototype.set = function (sid, sess, fn) {
    ...
    try {
      var jsess = store.serializer.stringify((store.secret) ?
        encryptData.call(store, store.serializer.stringify(sess), store.secret,
                         store.algorithm) : sess);
    }
    catch (er) {
      return fn(er);
    }
